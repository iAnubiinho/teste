...
policeAlert = function(coords)
    if Config['CasinoHeist']["dispatch"] == "default" then
        TriggerServerEvent('casinoheist:server:policeAlert', coords)
    elseif Config['CasinoHeist']["dispatch"] == "cd_dispatch" then
        local data = exports['cd_dispatch']:GetPlayerInfo()
        TriggerServerEvent('cd_dispatch:AddNotification', {
            job_table = Config["CasinoHeist"]['dispatchJobs'], 
            coords = coords,
            title = 'Casino Robbery',
            message = 'A '..data.sex..' robbing a Casino at '..data.street, 
            flash = 0,
            ...
            blip = {
                ...
                colour = 3,
                flashes = false, 
                text = '911 - Casino Robbery',
                time = 5,
                radius = 0,
            }
        })
    elseif Config['CasinoHeist']["dispatch"] == "qs-dispatch" then
        local playerData = exports['qs-dispatch']:GetPlayerInfo()
        TriggerServerEvent('qs-dispatch:server:CreateDispatchCall', {
            job = Config["CasinoHeist"]['dispatchJobs'],
            callLocation = coords,
    ...
    elseif Config['CasinoHeist']["dispatch"] == "ps-dispatch" then
        local dispatchData = {
            message = "Casino Robbery",
            codeName = 'casino',
            code = '10-90',
            icon = 'fas fa-store',
            priority = 2,
            ...
            street = "Casino",
            camId = nil,
            jobs = Config["CasinoHeist"]['dispatchJobs'],
        }
        TriggerServerEvent('ps-dispatch:server:notify', dispatchData)
    elseif Config['CasinoHeist']["dispatch"] == "rcore_dispatch" then
        local data = {
            ...
            job = Config["CasinoHeist"]['dispatchJobs'], -- string | table -> The job, for example 'police' or a table {'police', 'ambulance'}
            text = 'Casino Robbery', -- string -> The alert text
            ...
            blip = { -- Blip table (optional)
                ...
                text = 'Casino Robbery', -- number (optional) -> The blip text
