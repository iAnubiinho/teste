...
function CardSwipe()
    ESX.TriggerServerCallback('casinoheist:server:checkPoliceCount', function(status)
        if status then
...
AddEventHandler('casinoheist:client:npcSync', function()
    GuardPeds()
    policeAlert(Config['CasinoHeist']['startHeist']['cardSwipe']['scenePos'])
end)
...
function Nightvision()
    ...
    while true do
        ...
        if dist <= 3.0 and nightvision == false then
...
RegisterNetEvent('casinoheist:client:showNotification', function(str)
