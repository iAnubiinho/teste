...
RegisterNetEvent('bettaheist:client:showNotification')
AddEventHandler('bettaheist:client:showNotification', function(str)
    ShowNotification(str)
end)

--This event send to all police players
RegisterNetEvent('bettaheist:client:policeAlert')
AddEventHandler('bettaheist:client:policeAlert', function(targetCoords)
    ShowNotification(Strings['police_alert'])
    local alpha = 250
    local bettaBlip = AddBlipForRadius(targetCoords.x, targetCoords.y, targetCoords.z, 50.0)

    SetBlipHighDetail(bettaBlip, true)
    SetBlipColour(bettaBlip, 1)
    SetBlipAlpha(bettaBlip, alpha)
    SetBlipAsShortRange(bettaBlip, true)
