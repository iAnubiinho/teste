...
Citizen.CreateThread(function()
    if Config['DrugBoatHeist']['framework']['name'] == 'ESX' then
        while not ESX do
            pcall(function() ESX = exports[Config['DrugBoatHeist']['framework']['scriptName']]:getSharedObject() end)
            if not ESX then
    ...
    elseif Config['DrugBoatHeist']['framework']['name'] == 'QB' then
        while not QBCore do
            ...
            if not QBCore then
                TriggerEvent(Config['DrugBoatHeist']['framework']['eventName'], function(obj) QBCore = obj end)
            end
            Citizen.Wait(1)
        end
    end
...
AddEventHandler('drugboatheist:client:mainLoop', function(cfg, netId)
    ...
    while mainLoop do
        ...
        if dist <= 100.0 then
            ...
            if robber and Config['DrugBoatHeist']['buyerFinishScene'] then
                Outside()
                break
            end
        end
...
function GrabDrugs(index, type)
    ...
    TriggerCallback('drugboatheist:server:hasItem', function(data)
        if data['status'] then
            ...
            Wait(1000)
            SetPedComponentVariation(ped, 5, Config['DrugBoatHeist']['bagClothesID'], 0, 0)
